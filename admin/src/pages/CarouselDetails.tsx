...
export default function Index() {
  ...
  return (
    <div className="min-h-screen ">
      ...
      {/* Hero Banner Image - First artwork image */}
      <div
        ...
        <div className="flex flex-col lg:flex-row gap-16 py-8 pb-24">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-8">
            ...
            <div className="flex flex-wrap gap-6 p-4 sm:px-6 py-4 rounded-xl border border-gray-700 bg-gray-800 overflow-hidden">
              <StatItem
                label="Views Overtime"
                value={carousel.views.toLocaleString()}
              />
              <StatItem
                label="Completion rate"
                value={`${carousel.completionRate}%`}
              />
              <StatItem
                label="Average view duration"
                value={`${carousel.averageViewDuration}mins`}
              />
            </div>
            ...
            {/* Preview - Grid with first image larger, others smaller */}
            <section className="flex flex-col gap-3">
              <h2 className="text-base font-medium leading-6 text-white">
                Preview
              </h2>
              <div className="grid grid-cols-1 gap-3 sm:gap-4">
                ...
                {/* {allImages.length > 0 && (
                  <div
                    className="rounded-lg overflow-hidden aspect-[16/9] bg-surface cursor-pointer hover:opacity-80 transition-opacity"
            ...
            {carousel.tag && carousel.tag.length > 0 && (
              <section className="flex flex-col gap-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {carousel.tag.map((tag) => (
                    <span
                      key={tag}
                      className="inline-flex items-center px-[6px] py-[2px] rounded-md border border-gray-700 bg-gray-800 text-gray-400 text-xs font-medium leading-[18px]"
                    >
                      {tag}
                    </span>
                  ))}
...
function StatItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col gap-2 flex-1 min-w-[130px]">
      ...
      <span className="text-base font-semibold leading-6 text-[#d8522e]">
        {value}
      </span>
    </div>
  );
