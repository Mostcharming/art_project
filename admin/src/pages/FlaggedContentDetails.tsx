...
import DismissReportModal from "../components/moderation/DismissReportModal";
import RemoveContentModal from "../components/moderation/RemoveContentModal";
...
export default function FlaggedContentDetailsPage() {
  ...
  const dismissMutation = useApiMutation({
    endpoint: `/admins/carousels/${carouselId}/dismiss`,
    method: "POST",
  });

  const removeMutation = useApiMutation({
    endpoint: `/admins/carousels/${carouselId}/remove`,
    method: "POST",
  });
  ...
  const flaggedCount = (carousel as unknown as Record<string, unknown>)
    ?.flaggedCount as number | undefined;
  const flaggedReason = (carousel as unknown as Record<string, unknown>)
    ?.flaggedReason as string | undefined;
  const additionalReason = (carousel as unknown as Record<string, unknown>)
    ?.additionalReason as string | undefined;

  const handleUnpublish = async () => {
    try {
      setUnpublishLoading(true);
      toast.success(
        "Content has been made private. It is no longer visible to viewers."
      );
      moderation.refetchAll();
      // setTimeout(() => navigate("/content"), 1000);
    } catch (error) {
      console.error("Error unpublishing carousel:", error);
  ...
  const handleDismissProceed = async (data: { unflaggedReason: string }) => {
    try {
      setDismissLoading(true);
      await dismissMutation.mutateAsync(data);
      toast.success("Report dismissed successfully. Carousel is now active.");
      moderation.refetchAll();
      navigate("/content");
    } catch (error) {
  ...
  const handleRemoveProceed = async (data: { removalReason: string }) => {
    try {
      ...
      toast.error("Failed to remove carousel");
    } finally {
    ...
    <div className="min-h-screen">
      ...
      <div className="w-full max-w-[1280px] mx-auto px-4 sm:px-8">
        ...
        <div className="flex flex-wrap items-start gap-4 pt-6 pb-5">
          ...
          <div className="flex flex-col sm:flex-row items-stretch gap-2 sm:gap-3">
            <button
              ...
              disabled={unpublishLoading || dismissLoading || removeLoading}
              className="flex items-center gap-1 px-[14px] py-[10px] rounded-lg border border-gray-500 bg-gray-800 text-gray-300 text-sm font-semibold leading-5 hover:opacity-80 transition-opacity disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {unpublishLoading
                ? "Unpublishing..."
                : "Temporarily Unpublish content"}
            </button>
            <button
              onClick={handleDismissClick}
              disabled={unpublishLoading || dismissLoading || removeLoading}
              className="flex items-center justify-center px-3 py-2 rounded-lg bg-[#D8522E] text-white text-sm font-semibold leading-5 hover:opacity-90 transition-opacity"
            >
              {dismissLoading ? "Dismissing..." : "Dismiss Report"}
            </button>
            <button
        ...
        <div className="flex flex-col lg:flex-row gap-16 py-8 pb-24">
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-8">
            {/* Stats card - Updated for Flagged Content */}
            <div className="flex flex-wrap gap-6 p-4 sm:px-6 py-4 rounded-xl border border-gray-700 bg-gray-800 overflow-hidden">
              <StatItem
                label="Date Reported"
                value={new Date(
                  (carousel?.createdAt as unknown as string) ||
                    new Date().toISOString()
                ).toLocaleDateString()}
              />
              <StatItem
                label="Report Count"
                value={flaggedCount?.toString() || "1"}
              />
              <StatItem
                label="Report Reason"
                value={flaggedReason || "Unknown"}
              />
            </div>

            {/* Detailed Report Reasons */}
            <section className="flex flex-col gap-3">
              <h2 className="text-base font-medium leading-6 text-white">
                Detailed Report Reasons
              </h2>
              <div className="text-base font-normal leading-6 text-gray-400 p-4 rounded-lg bg-gray-800/50 border border-gray-700">
                <p className="font-semibold text-white mb-2">
                  Flagged Reason: {flaggedReason || "Not specified"}
                </p>
                <p>
                  {additionalReason || "No additional information provided"}
                </p>
              </div>
            </section>
      ...
      {showDismissModal && (
        <DismissReportModal
          ...
          onProceed={handleDismissProceed}
