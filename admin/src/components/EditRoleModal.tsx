...
export default function EditRoleModal({
  ...
  // Update role privileges mutation (for custom roles)
  const updateRolePrivilegesMutation = useApiMutation({
    endpoint: selectedRoleId ? `/admins/roles/${selectedRoleId}` : "",
    method: "PUT",
  });
  ...
  const isFormValid =
    selectedRoleId && (!isCustomRole || hasSelectedPermissions);

  // Check if anything has changed
  const hasRoleChanged = selectedRoleId !== admin.roleId;
  ...
  const isSubmitDisabled = !hasRoleChanged && !hasPermissionsChanged;
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      // Update role privileges if it's a custom role and privileges changed
      if (hasPermissionsChanged && isCustomRole && selectedRoleId) {
        await updateRolePrivilegesMutation.mutateAsync({
          privilegeIds: selectedPermissions,
        } as never);
      }
    ...
    return (
      <div className="relative w-full max-w-[500px] rounded-2xl bg-[#0C111D] p-6 flex flex-col gap-8 items-center text-center animate-modal-in">
        ...
        <p className="text-[#A0A4A8] text-base font-normal leading-6">
          The team member's role and permissions have been updated
        </p>
    ...
    <div className="relative w-full max-w-[664px] rounded-2xl bg-[#0C111D] p-6 flex flex-col gap-8 animate-modal-in">
      ...
      <button
        ...
        disabled={
          ...
          updateRolePrivilegesMutation.isPending
        }
      ...
      >
        {updateAdminMutation.isPending || updateRolePrivilegesMutation.isPending
          ? "Saving..."
          : "Save Changes"}
      </button>
    </div>
