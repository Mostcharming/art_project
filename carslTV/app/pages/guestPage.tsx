          ...
          >
            <Text
              style={{
                fontSize: 16,
                fontWeight: "500",
                color: item.focused ? "white" : "rgba(255, 255, 255, 0.7)",
              }}
            >
              {item.label}
            </Text>
...
type NavigationFocus =
  | "navbar" // 0-4: nav items
  | "play" // 5: play carousel button
  | "favorite" // 6: add to favorites button
  | "trending" // 7: trending items
  | "artists"; // 8: artist items

function GuestScreenContent() {
  const router = useRouter();
  const [navigationFocus, setNavigationFocus] =
    useState<NavigationFocus>("navbar");
  ...
  useTVRemote({
    onUp: () => {
      if (navigationFocus === "trending") {
        setNavigationFocus("play");
      } else if (navigationFocus === "artists") {
        setNavigationFocus("trending");
      } else if (navigationFocus === "play" || navigationFocus === "favorite") {
        setNavigationFocus("navbar");
      }
    },
    onDown: () => {
      if (navigationFocus === "navbar") {
        ...
        setNavigationFocus("trending");
      } else if (navigationFocus === "trending") {
        setNavigationFocus("artists");
      }
    },
    onLeft: () => {
      if (navigationFocus === "navbar") {
        setSelectedNavIndex((prev) => Math.max(0, prev - 1));
      } else if (navigationFocus === "trending" && selectedTrendingCard) {
        const currentIndex = trendingItems.findIndex(
          (item) => item.id === selectedTrendingCard
        );
        if (currentIndex > 0) {
          setSelectedTrendingCard(trendingItems[currentIndex - 1].id);
        }
      } else if (navigationFocus === "artists" && selectedArtistCard) {
        const currentIndex = artistItems.findIndex(
          (item) => item.id === selectedArtistCard
        );
        if (currentIndex > 0) {
          setSelectedArtistCard(artistItems[currentIndex - 1].id);
        }
      } else if (navigationFocus === "favorite") {
        setNavigationFocus("play");
      }
    },
    onRight: () => {
      if (navigationFocus === "navbar") {
        setSelectedNavIndex((prev) => Math.min(4, prev + 1));
      } else if (navigationFocus === "trending" && trendingItems.length > 0) {
        if (!selectedTrendingCard) {
          setSelectedTrendingCard(trendingItems[0].id);
        } else {
          const currentIndex = trendingItems.findIndex(
            (item) => item.id === selectedTrendingCard
          );
      ...
      } else if (navigationFocus === "artists" && artistItems.length > 0) {
        if (!selectedArtistCard) {
          setSelectedArtistCard(artistItems[0].id);
        } else {
          const currentIndex = artistItems.findIndex(
            (item) => item.id === selectedArtistCard
          );
          if (currentIndex < artistItems.length - 1) {
            setSelectedArtistCard(artistItems[currentIndex + 1].id);
          }
        }
      } else if (navigationFocus === "play") {
        setNavigationFocus("favorite");
      }
    },
    onSelect: () => {
      if (navigationFocus === "navbar") {
        if (selectedNavIndex === 4) {
          ...
          // Other nav items show sign in modal
      ...
      <ScrollView className="flex-1 bg-black">
        ...
        >
          <View className="relative z-10 flex flex-col">
            <Navbar
              selectedNavIndex={
                selectedNavIndex >= 0 && selectedNavIndex < 5
                  ? selectedNavIndex
                  : undefined
              }
              onSignUpPress={() => {
        ...
        <View className="">
          <ContentRow
            ...
            accent
            selectedCardId={
              navigationFocus === "trending" ? selectedTrendingCard : undefined
            }
            onCardSelect={(id) => {
              setSelectedTrendingCard(id);
              setNavigationFocus("trending");
            }}
            onCardPress={() => {
              setNavigationFocus("trending");
          ...
          <ContentRow
            ...
            accent
            selectedCardId={
              navigationFocus === "artists" ? selectedArtistCard : undefined
            }
            onCardSelect={(id) => {
              setSelectedArtistCard(id);
              setNavigationFocus("artists");
            }}
            onCardPress={() => {
              setNavigationFocus("artists");
