  ...
  useTVRemote({
    onLeft: () => {
      if (focusedElement === "buttons") {
        setSelectedButtonId(selectedButtonId === "guest" ? "signin" : "guest");
      }
    },
    onRight: () => {
      if (focusedElement === "buttons") {
    ...
    onUp: () => {
      // Move up from sign-in to buttons
      if (focusedElement === "signin") {
        setFocusedElement("buttons");
      }
    },
    onDown: () => {
      // Move down from buttons to sign-in
      if (focusedElement === "buttons") {
        setFocusedElement("signin");
      }
    },
    onSelect: () => {
      // Handle selection based on focused element
      if (focusedElement === "buttons") {
            ...
            <View className="flex-row gap-4 justify-center flex-wrap">
              {BUTTONS.map((button) => (
                <Pressable
                  ...
                  className="h-12 rounded-lg items-center justify-center px-6 border-2 transition-all duration-200"
                  style={{
                    backgroundColor:
              ...
              style={{
                ...
                backgroundColor:
                  focusedElement === "signin"
                    ? "rgba(216, 82, 46, 0.2)"
                    : "transparent",
                borderWidth: focusedElement === "signin" ? 2 : 0,
                borderColor:
                  focusedElement === "signin" ? "#D8522E" : "transparent",
                shadowColor:
                  focusedElement === "signin" ? "#D8522E" : "transparent",
                shadowOpacity: focusedElement === "signin" ? 0.5 : 0,
                shadowRadius: focusedElement === "signin" ? 4 : 0,
            ...
            >
              <Text className="text-center text-[#D2D6DB] text-base">
                ...
                <Text
                  className={
                    focusedElement === "signin"
                      ? "text-[#D8522E] font-bold"
                      : "text-[#D8522E] font-semibold"
                  }
                >
                  Sign In
                </Text>
